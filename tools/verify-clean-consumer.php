...
$workspace = rtrim(sys_get_temp_dir(), '/\\') . '/kumwe-computation-consumer-' . bin2hex(random_bytes(6));
/**
...
consumerRun(
    [
        ...
        '--format=zip',
        '--dir=' . $distDirectory,
        '--file=kumwe-computation',
    ],
    $workspace,
);
$archive = $distDirectory . '/kumwe-computation.zip';
if (!is_file($archive)) {
...
$installed = $consumer . '/vendor/kumwe/computation';
...
foreach (array_keys($classmap) as $symbol) {
    if (
        ...
        || str_starts_with($symbol, 'Kumwe\\Computation\\Tests\\')
    ) {
