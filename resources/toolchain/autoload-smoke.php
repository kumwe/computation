...
if (PHP_INT_SIZE !== 8) {
    fwrite(STDERR, "Computation requires 64-bit PHP for its signed integer transport.\n");
    exit(1);
}
 ...
 * @since   0.1.0
 */
function computationSmokeDeclaringFile(string $name, mixed $kind): ?string
{
...
foreach ($symbols as $name => $shape) {
    ...
    $declaredIn = computationSmokeDeclaringFile($name, $shape['kind'] ?? null);
    if ($declaredIn === null) {
