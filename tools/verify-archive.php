 ...
 *
 * The Kumwe App adoption gate reads CHARTER.md, README.md, CHANGELOG.md, docs/release-record.md, docs/,
...
$requiredRoots = [
    'CHANGELOG.md', 'CHARTER.md', 'LICENSE', 'docs/release-record.md', 'README.md', 'SECURITY.md', 'composer.json',
];
$requiredShipped = [
...
foreach ($symbols as $symbol => $entry) {
    ...
    if (
        !is_string($symbol)
        || !str_starts_with($symbol, 'Kumwe\\Computation\\')
        || !is_string($file)
...
if (($composer['name'] ?? null) !== 'kumwe/computation') {
    $errors[] = 'The archived composer.json does not name kumwe/computation.';
}
